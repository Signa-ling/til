...
2. トピック名を特定して適切なファイル名を生成（kebab-case形式）
...
- ファイル名はkebab-case形式（小文字、ハイフン区切り）
- タイトルはファイル名から生成する（ハイフンをスペースに置換し、各単語の先頭を大文字にする）
  - 例：`method-missing.md` → `# Method Missing`
  - 例：`type-guard.md` → `# Type Guard`
